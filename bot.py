...
"""
Telegram Video Downloader Bot - الإصدار النهائي
Version: 4.0.0
"""
...
# التحقق من التوكن
if not BOT_TOKEN or BOT_TOKEN == "7536390168:AAHZNO7XjIRBpwhMf3O5RojM9f2RrPYzUZ4":
    print("=" * 50)
    print("❌ خطأ: التوكن غير صحيح!")
    print("📌 اذهب إلى @BotFather في تليجرام")
    print("📌 أنشئ بوت جديد واحصل على التوكن")
    print("📌 ثم غيره في ملف bot.py")
    print("=" * 50)
    sys.exit(1)
...
class VideoProcessor:
    ...
    async def search_videos(self, query, limit=5):
        try:
            ...
            with yt_dlp.YoutubeDL({**self.ydl_opts, 'extract_flat': True}) as ydl:
                results = await loop.run_in_executor(None, lambda: ydl.extract_info(f"ytsearch{limit}:{query}", download=False))
                videos = []
    ...
    async def download_video(self, url, quality='best', format='mp4'):
        try:
            if quality == 'best':
                format_spec = 'best[ext=mp4]/best' if format != 'mp3' else 'bestaudio/best'
            else:
                format_spec = f'best[height<={quality}][ext=mp4]/best' if format != 'mp3' else 'bestaudio/best'
...
👥 *عدد المستخدمين:* {self.users_count}
...
👇 *اختر ما تريد:*
        ...
        logger.info(f"مستخدم جديد: {user.first_name}")
...
/help - هذه المساعدة
    ...
    async def browse_category(self, update: Update, context: ContextTypes.DEFAULT_TYPE, category):
        ...
        videos = await self.processor.search_videos(VIDEO_CATEGORIES[category]['query'], limit=5)
